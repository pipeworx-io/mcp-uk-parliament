...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'UK Parliament');
}
...
const tools: McpToolExport['tools'] = [
  ...
  {
    name: 'get_bill',
    description: 'Fetch full detail for a UK Parliament bill by numeric bill_id — returns title, short title, sponsors, current stage, originating house, sessions, and summary description.',
    inputSchema: {
      type: 'object',
  ...
  },
  {
    ...
    description: 'Recent divisions (recorded votes) in the UK Parliament, read from the official Hansard API at hansard-api.parliament.uk, filterable by house (Commons or Lords) and a start date, up to 25 per call. Returns each division title, date, house, and division id. Answers what the House of Commons or House of Lords has voted on lately at Westminster.',
    inputSchema: {
      type: 'object',
      properties: {
        house: { type: 'string', description: 'Commons | Lords' },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'search_hansard': {
      ...
      if (args.member_id !== undefined) params.set('memberId', String(args.member_id));
      // Hansard requires the `.json` extension AND is case-sensitive on the
      // contribution type: `/contributions/Spoken.json` works, `spoken.json`
      // 500s and the extensionless form 404s.
      return ukpGet(`${HANSARD}/search/contributions/Spoken.json?${params}`);
    }
    case 'recent_divisions': {
      ...
      if (args.house) params.set('house', String(args.house));
      if (args.date_from) params.set('startDate', String(args.date_from));
      return ukpGet(`${HANSARD}/search/divisions.json?${params}`);
    }
    default:
