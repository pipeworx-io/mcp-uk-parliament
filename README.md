...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/search_members \
  -H 'Content-Type: application/json' \
  -d '{"name":"Keir Starmer","is_current":true}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/search_members`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "uk-parliament": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-uk-parliament"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-uk-parliament
```
